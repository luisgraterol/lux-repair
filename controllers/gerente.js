    ...
    try {
        ...
        for (let i = 0; i < vehiculos.length; i++) {
            ...
            if (!!response) { // Solo agrega los datos si ese vehiculo tiene una orden activa
                ...
                vehiculos[i].Imagen = response.dataValues.Imagen;
            }
        }
...
// Metodo que retorna un arreglo de vehiculos con clientes
controller.getVehiculosConCliente = async function (callback) {
    try {
        let response = await Vehiculo.findAll({ where: { Activo: true } });
        ...
        let clientes = usuarios.map(usuario => {
            return {
        ...
        // Retorna el arreglo
        callback({vehiculos, clientes}, null);
    } catch (err) {
        callback(null, err);
    }
};
...
// Metodo que retorna un arreglo de vehiculos con todos los datos pertinentes
controller.getVehiculosSinFiltro = async function (callback) {
    try {
        let response = await Vehiculo.findAll({ where: { Activo: true } });
